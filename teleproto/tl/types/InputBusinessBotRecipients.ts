...
export class InputBusinessBotRecipients extends TLObject {
    static CONSTRUCTOR_ID = 3303379486;
    static SUBCLASS_OF_ID = 2849240411;
    static className = "InputBusinessBotRecipients";
    ...
    constructor(args: { flags?: number, existingChats?: boolean, newChats?: boolean, contacts?: boolean, nonContacts?: boolean, excludeSelected?: boolean, users?: TypeInputUser[], excludeUsers?: TypeInputUser[] } = {}) {
        super();
    ...
    getBytes(): Buffer {
        ...
        writer.writeInt(3303379486, false);
        let flags = 0;
        ...
        if (this.excludeUsers !== undefined && this.excludeUsers !== null) { flags |= 1 << 6; }
        ...
        if (this.excludeUsers !== undefined && this.excludeUsers !== null) {
            writer.writeVector(this.excludeUsers, (item) => {
                writer.write(item.getBytes());
            });
        }
    ...
    static fromReader(reader: BinaryReader): InputBusinessBotRecipients {
        const args: any = {};
        ...
        if (args.flags & (1 << 6)) {
            const _excludeUsers = reader.readVector((reader) => {
                const item = reader.tgReadObject();
                return item;
            });
            args.excludeUsers = _excludeUsers;
        } else {
            args.excludeUsers = undefined;
        }
        return new InputBusinessBotRecipients(args);
    }
}