import { BinaryReader } from "../../extensions/BinaryReader";
import { BinaryWriter } from "../../extensions/BinaryWriter";
import { TLObject } from "../../extensions/TLObject";
import { TypeInputUser } from "./TypeInputUser";

export class InputStorePaymentGiftPremium extends TLObject {
    static CONSTRUCTOR_ID = 1634697192;
    static SUBCLASS_OF_ID = 3886290765;
    static className = "InputStorePaymentGiftPremium";
    static classType = "constructor";

    userId!: TypeInputUser;
    currency!: string;
    amount!: bigint;

    constructor(args: { userId?: TypeInputUser, currency?: string, amount?: bigint } = {}) {
        super();
        this.userId = args.userId!;
        this.currency = args.currency!;
        this.amount = args.amount!;
    }

    getBytes(): Buffer {
        const writer = new BinaryWriter(Buffer.alloc(0));
        writer.writeInt(1634697192, false);
        writer.write(this.userId.getBytes());
        writer.tgWriteString(this.currency);
        writer.writeLargeInt(this.amount, 64);
        return writer.getValue();
    }

    static fromReader(reader: BinaryReader): InputStorePaymentGiftPremium {
        const args: any = {};
        const _userId = reader.tgReadObject();
        args.userId = _userId;
        const _currency = reader.tgReadString();
        args.currency = _currency;
        const _amount = reader.readLargeInt(64);
        args.amount = _amount;
        return new InputStorePaymentGiftPremium(args);
    }
}