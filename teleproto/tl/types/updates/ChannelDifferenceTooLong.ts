import { BinaryReader } from "../../../extensions/BinaryReader";
import { BinaryWriter } from "../../../extensions/BinaryWriter";
import { TLObject } from "../../../extensions/TLObject";
import { TypeDialog } from "../TypeDialog";
import { TypeMessage } from "../TypeMessage";
import { TypeChat } from "../TypeChat";
import { TypeUser } from "../TypeUser";

export class ChannelDifferenceTooLong extends TLObject {
    static CONSTRUCTOR_ID = 2763835134;
    static SUBCLASS_OF_ID = 696872797;
    static className = "updates.ChannelDifferenceTooLong";
    static classType = "constructor";

    flags!: number;
    final?: boolean;
    timeout?: number;
    dialog!: TypeDialog;
    messages!: TypeMessage[];
    chats!: TypeChat[];
    users!: TypeUser[];

    constructor(args: { flags?: number, final?: boolean, timeout?: number, dialog?: TypeDialog, messages?: TypeMessage[], chats?: TypeChat[], users?: TypeUser[] } = {}) {
        super();
        this.flags = args.flags!;
        this.final = args.final;
        this.timeout = args.timeout;
        this.dialog = args.dialog!;
        this.messages = args.messages!;
        this.chats = args.chats!;
        this.users = args.users!;
    }

    getBytes(): Buffer {
        const writer = new BinaryWriter(Buffer.alloc(0));
        writer.writeInt(2763835134, false);
        let flags = 0;
        if (this.final) { flags |= 1 << 0; }
        if (this.timeout !== undefined && this.timeout !== null) { flags |= 1 << 1; }
        writer.writeInt(flags, false);
        if (this.final !== undefined && this.final !== null) {
        }
        if (this.timeout !== undefined && this.timeout !== null) {
            writer.writeInt(this.timeout);
        }
        writer.write(this.dialog.getBytes());
        writer.writeVector(this.messages, (item) => {
            writer.write(item.getBytes());
        });
        writer.writeVector(this.chats, (item) => {
            writer.write(item.getBytes());
        });
        writer.writeVector(this.users, (item) => {
            writer.write(item.getBytes());
        });
        return writer.getValue();
    }

    static fromReader(reader: BinaryReader): ChannelDifferenceTooLong {
        const args: any = {};
        const _flags = reader.readInt();
        args.flags = _flags;
        if (args.flags & (1 << 0)) {
            const _final = true;
            args.final = _final;
        } else {
            args.final = false;
        }
        if (args.flags & (1 << 1)) {
            const _timeout = reader.readInt();
            args.timeout = _timeout;
        } else {
            args.timeout = undefined;
        }
        const _dialog = reader.tgReadObject();
        args.dialog = _dialog;
        const _messages = reader.readVector((reader) => {
            const item = reader.tgReadObject();
            return item;
        });
        args.messages = _messages;
        const _chats = reader.readVector((reader) => {
            const item = reader.tgReadObject();
            return item;
        });
        args.chats = _chats;
        const _users = reader.readVector((reader) => {
            const item = reader.tgReadObject();
            return item;
        });
        args.users = _users;
        return new ChannelDifferenceTooLong(args);
    }
}